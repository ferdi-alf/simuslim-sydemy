...
<div class="w-full bg-white/35 backdrop-blur-3xl" data-table-container="{{ $uniqueTableId }}">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-{{ $position }} text-gray-600" id="enhanced-table-{{ $uniqueTableId }}">
            <thead class="text-xs text-gray-700 uppercase bg-gray-100/20 ">
                <tr>
            ...
            <tbody id="table-body-{{ $uniqueTableId }}">
                @foreach ($data as $index => $row)
                    <tr class="bg-white/25  border-b table-row" data-index="{{ $index }}">
    ...
    @if (!$disablePagination)
        <div
            class="flex overflow-x-auto items-center md:flex-row justify-between px-6 py-3 bg-white/30 backdrop-blur-3xl">
