...
@section('content')
    ...
    <x-fragments.form-modal id="add-bacaan-modal" title="Tambah Bacaan" action="{{ route('symuslim.store') }}">
        <div class="grid grid-cols-2 gap-2">
            ...
            <x-fragments.select-field label="Type" name="type" :options="['doa' => 'Doa', 'hadits' => 'Hadits', 'dzikir' => 'Dzikir']" required />
    ...
    @foreach ($bacaans as $bacaan)
        <x-fragments.form-modal id="edit-bacaan-modal-{{ $bacaan->id }}" title="Edit Bacaan"
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <x-fragments.select-field label="Type" name="type" :options="['doa' => 'Doa', 'hadits' => 'Hadits', 'dzikir' => 'Dzikir']" :value="$bacaan->type" required />
                <div class="col-span-2">
                    <x-fragments.textarea-field label="Deskripsi" name="deskripsi" :value="$bacaan->deskripsi" rows="4" />
                </div>
            </div>
        </x-fragments.form-modal>
            ...
            <x-fragments.form-modal id="edit-detail-modal-{{ $detail->id }}" title="Edit Detail Bacaan"
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="col-span-2">
                        ...
                        <x-fragments.textarea-field label="Terjemahan" name="terjemahan" :value="$detail->terjemahan" rows="3" />
        ...
        <div class="mt-6">
            ...
            @forelse ($bacaans as $bacaan)
                <div class="border border-gray-200 rounded-lg bg-white/25 backdrop-blur-2xl shadow-sm"
                        ...
                        <table class="w-full table-auto">
                            ...
                            <tbody>
                                <tr class="hover:bg-gray-50 transition-colors cursor-pointer">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <span
                                            class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{{ ucfirst($bacaan->type) }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-600 line-clamp-2">
                                        {{ $bacaan->deskripsi ?? 'Tidak ada' }}</td>
                    ...
                    <div x-show="isOpen" class="overflow-auto" x-transition:enter="transition ease-out duration-200"
